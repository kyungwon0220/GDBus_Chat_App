    ...
    if (serverData.user_map.find(bus_name) != serverData.user_map.end()) {
        ...
        if (!chat_title.empty()) {
            ...
            this->UserTyping_emitter(destinations, Glib::ustring(user_name));             // 본 함수를 호출한 클라이언트의 대화명을 신호로 보냄
                                                    ...
                                                    // destinations Debug ~
                                                    std::cout << "Sending 'UserTyping' signal to the following destinations:" << std::endl;
...
void MessageStub::UserStopTyping(MethodInvocation &invocation) {
    auto bus_name = invocation.getMessage()->get_sender();  // 클라이언트의 버스 이름을 얻음
    ...
    if (serverData.user_map.find(bus_name) != serverData.user_map.end()) {
        ...
        if (!chat_title.empty()) {
            ...
            this->UserStoppedTyping_emitter(destinations, Glib::ustring(user_name));    // 본 함수를 호출한 클라이언트의 대화명을 신호로 보냄

                                                    // destinations Debug ~
                                                    std::cout << "Sending 'UserStoppedTyping' signal to the following destinations:" << std::endl;
                                                    for (const auto &destination : destinations) {
                                                        std::cout << "  Destination: " << destination << std::endl;
                                                    }
                                                    // ~ destinations Debug
    ...
    for (const auto &entry : serverData.user_map) {
        if (std::get<1>(entry.second) == chat_title) {                          // 본 함수를 호출한 클라이언트와, 같은 채팅방에 접속중인 경우 ( 본인 포함 )
            destinations.push_back(Glib::ustring(entry.first));
                                            ...
                                            // destinations Debug ~
                                            std::cout << "Sending 'NewMsgReceived' signal to the following destinations:" << std::endl;
                                            for (const auto &destination : destinations) {
                                                std::cout << "  Destination: " << destination << std::endl;
                                            }
                                            // ~ destinations Debug
